import { CFG, IMAGES, IMAGE_CAPTIONS, THANKS_MESSAGES, MOVIE_QUOTES, IMAGE_COUNT, SPREAD_MOVIE_START, SPREAD_MOVIE_COUNT, SPREAD_IMAGE_START, SPREAD_MUSIC, SPREAD_NOTES, SPREAD_CLOSING, ROMANTIC_PHRASES } from '../../config';
import type { PageData } from '../../types/book';

let pageImgData: PageData[] = [];
let lightboxOpenFn: ((idx: number) => void) | null = null;

export function setLightboxOpenFn(fn: (idx: number) => void): void {
  lightboxOpenFn = fn;
}

export function initPageImages(): void {
...
export function getPageImgData(): PageData[] {
  return pageImgData;
}

function $(sel: string, ctx?: Document | HTMLElement): HTMLElement | null {
  return (ctx || document).querySelector<HTMLElement>(sel);
}

function cloneTemplate(selector: string): DocumentFragment | null {
  const tmpl = $(selector) as HTMLTemplateElement | null;
  if (!tmpl || !tmpl.content) return null;
  try { return tmpl.content.cloneNode(true) as DocumentFragment; }
  catch { return null; }
}

function injectPhrase(container: HTMLElement | DocumentFragment, pageIndex: number): void {
...
function createLetterPage(html: string, phraseIdx?: number): DocumentFragment {
...
function createMoviePage(quote: string, char: string, film: string, phraseIdx?: number): DocumentFragment | null {
...
function createImagePage(imgIdx: number): DocumentFragment | null {
...
function createThanksPage(imgIdx: number): DocumentFragment {
  const frag = document.createDocumentFragment();
  const div = document.createElement('div');
...
export function createPageContent(pageIndex: number): DocumentFragment | null {
...
export function hasPageContent(pageIndex: number): boolean {
